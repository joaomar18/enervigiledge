        ...
        async def wrapper(request: Request, safety: HTTPSafety, **kwargs) -> JSONResponse:
            ...
            try:
                ...
                if safety.is_blocked(request):
                    return JSONResponse(
                        status_code=429, content={"unlocked": safety.get_unlocked_date(request), "error": "Too many failed attempts. Try again later."}
                    )
                ...
                safety.clean_failed_requests(request, request.url.path)  # Clean failed requests on success
                ...
                logger.exception(f"Failed {request.url.path} API due to server error: {str(e)}")
