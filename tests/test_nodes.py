...
def test_incremental_node_logic():
    node = Node("energy", NodeType.FLOAT, "kWh", incremental_node=True)
    node.set_value(100)
    assert node.value == 0
    node.set_value(150)
...
def test_publish_format_with_alarms_and_rounding():
    node = Node("voltage", NodeType.FLOAT, "V", min_alarm=True, max_alarm=True, min_alarm_value=5.0, max_alarm_value=15.0, decimal_places=2)
    node.set_value(3.14159)
    publish = node.get_publish_format()
    assert publish["value"] == 3.14
    assert publish["type"] == NodeType.FLOAT.value
